...
impl Client for ElrondClient {
    async fn client_version(&self) -> Result<String> {
        ...
        match (resp.data, resp.error) {
            ...
            (serde_json::Value::Null, None) => {
                Err(anyhow!("missing error response"))
            }
            (data, _) => {
                Ok(data.get("config").ok_or_else(|| anyhow!("no config key"))?
                   .get("erd_latest_tag_software_version").ok_or_else(|| anyhow!("no erd_latest_tag_software_version key"))?
    ...
    async fn get_latest_block_number(&self) -> Result<u64> {
        ...
        match (resp.data, resp.error) {
            ...
            (data, _) => {
                Ok(data.get("status").ok_or_else(|| anyhow!("no status key"))?
                   .get("erd_highest_final_nonce").ok_or_else(|| anyhow!("no erd_highest_final_nonce key"))?
                   .as_u64().ok_or_else(|| anyhow!("not a u64"))?)
            }
        }
    }
    ...
    async fn get_block(&self, block_number: u64) -> Result<Option<Block>> {
        ...
        match (resp.data, resp.error) {
            ...
            (data, _) => {
                let block = data.get("hyperblock").ok_or_else(|| anyhow!("no hyperblock key"))?;
                log::info!("{:?}", block);
                todo!();
            }
        }
